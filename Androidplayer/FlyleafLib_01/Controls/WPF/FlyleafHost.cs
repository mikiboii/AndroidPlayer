...
using static FlyleafLib_01.Utils.NativeMethods;

// using FlyleafLib.MediaPlayer;

namespace FlyleafLib_01.Controls.WPF;
...
public class FlyleafHost : ContentControl, IHostPlayer, IDisposable
{
    ...
    public bool isSurfaceCreated = false;
    ...
    private class FlyleafHostDropWrap { public FlyleafHost FlyleafHost01; } // To allow non FlyleafHosts to drag & drop
    ...
    private static void OnMouseBindings(DependencyObject d, DependencyPropertyChangedEventArgs e)
    {
        ...
        FlyleafHost host01 = d as FlyleafHost;
        if (host01.Disposed)
            return;

        host01.SetMouseSurface();
        host01.SetMouseOverlay();
    }
    private static void OnDetachedTopMostChanged(DependencyObject d, DependencyPropertyChangedEventArgs e)
    {
        ...
        if (host01.Surface != null)
            host01.Surface.Topmost = !host01.IsAttached && host01.DetachedTopMost;
    }
    private static void DropChanged(DependencyObject d, DependencyPropertyChangedEventArgs e)
    {
        ...
        host01.Surface.AllowDrop =
            host01.OpenOnDrop == AvailableWindows.Surface || host01.OpenOnDrop == AvailableWindows.Both ||
            host01.SwapOnDrop == AvailableWindows.Surface || host01.SwapOnDrop == AvailableWindows.Both;

        if (host01.Overlay == null)
            return;

        host01.Overlay.AllowDrop =
            host01.OpenOnDrop == AvailableWindows.Overlay || host01.OpenOnDrop == AvailableWindows.Both ||
            host01.SwapOnDrop == AvailableWindows.Overlay || host01.SwapOnDrop == AvailableWindows.Both;
    }
    private static void OnShowInTaskBarChanged(DependencyObject d, DependencyPropertyChangedEventArgs e)
    {
        ...
        if (host01.DetachedShowInTaskbar)
            SetWindowLong(host01.SurfaceHandle, GetWindowLongEx(host01.SurfaceHandle) |  WindowStylesEx.WS_EX_APPWINDOW);
        else
            SetWindowLong(host01.SurfaceHandle, GetWindowLongEx(host01.SurfaceHandle) & ~WindowStylesEx.WS_EX_APPWINDOW);
    }
    private static void OnNoOwnerChanged(DependencyObject d, DependencyPropertyChangedEventArgs e)
    {
        ...
        if (!host01.IsAttached)
            host01.Surface.Owner = host01.DetachedNoOwner ? null : host01.Owner;
    }
    private static void OnKeepRatioOnResizeChanged(DependencyObject d, DependencyPropertyChangedEventArgs e)
    {
        ...
        host01._KeepRatioOnResize = (bool)e.NewValue;
        host01.ResizeRatio();
    }
    ...
    private static void OnIsFullScreenChanged(DependencyObject d, DependencyPropertyChangedEventArgs e)
    {
        ...
        host01._IsFullScreen = (bool)e.NewValue;
        host01.RefreshNormalFullScreen();
    }
    private static void OnIsMinimizedChanged(DependencyObject d, DependencyPropertyChangedEventArgs e)
    {
        ...
        if (host01.Surface != null)
            host01.Surface.WindowState = host01.IsMinimized ? WindowState.Minimized : (host01.IsFullScreen ? WindowState.Maximized : WindowState.Normal);
    }
    private static void OnIsAttachedChanged(DependencyObject d, DependencyPropertyChangedEventArgs e)
    {
        ...
        if (host01.IsStandAlone)
        {
            host01.IsAttached = false;
            return;
        }

        host01._IsAttached = (bool)e.NewValue;
    ...
    private static void OnOverlayTemplateChanged(DependencyObject d, DependencyPropertyChangedEventArgs e)
    {
        ...
        if (host01.Overlay == null)
        {
            host01.setTemplate= true;
            host01.Overlay    = new Window() { WindowStyle = WindowStyle.None, ResizeMode = ResizeMode.NoResize, AllowsTransparency = true };
            host01.setTemplate= false;
        }
        else
            host01.Overlay.Template = host01.OverlayTemplate;
    }
    ...
    private static void OnCornerRadiusChanged(DependencyObject d, DependencyPropertyChangedEventArgs e)
    {
        ...
        host01._CornerRadius = (CornerRadius)e.NewValue;
    ...
    private static void OnVideoBackgroundChanged(DependencyObject d, DependencyPropertyChangedEventArgs e)
    {
        ...
        if (host01.Surface != null)
            host01.Surface.Background = (Brush)e.NewValue;
    }
    private static object OnContentChanging(DependencyObject d, object baseValue)
    {
        ...
        FlyleafHost host01 = d as FlyleafHost;
        if (host01.Disposed)
            return host01.DetachedContent;

        if (baseValue != null && host01.Overlay == null)
            host01.Overlay = new Window() { WindowStyle = WindowStyle.None, ResizeMode = ResizeMode.NoResize, AllowsTransparency = true };

        if (host01.Overlay != null)
            host01.Overlay.Content = baseValue;
    ...
    private void Host_LayoutUpdated(object sender, EventArgs e)
    {
        ...
        try
        {
            rectInit = rectIntersect = new(TransformToAncestor(Owner).Transform(Utils.PointEmpty), RenderSize);
            ...
            while ((parent = VisualTreeHelper.GetParent(parent) as FrameworkElement) != null)
            {
                ...
                {
                    var location = parent.TransformToAncestor(Owner).Transform(Utils.PointEmpty);
                    location.X  -= parent.RenderSize.Width;
                    rectIntersect.Intersect(new(location, parent.RenderSize));
                }
                else
                    rectIntersect.Intersect(new(parent.TransformToAncestor(Owner).Transform(Utils.PointEmpty), parent.RenderSize));
            }
            ...
            else if (rectIntersectLast != Utils.RectZero)
            {
                rectIntersectLast = Utils.RectZero;
                SetVisibleRect(Utils.RectZero);
            }
        }
        catch (Exception ex)
        {
            ...
            Console.WriteLine($"Host_LayoutUpdated: {ex.Message}");
        ...
        // Console.WriteLine("Host_LayoutUpdated");
    ...
    public virtual void SetSurface(bool fromSetOverlay = false)
    {
        ...
        // Console.WriteLine("Invoking SurfaceCreated");
        SurfaceCreated?.Invoke(this, new());
        
        Dispatcher.BeginInvoke(new Action(() =>
        {
            HandleCreated?.Invoke(SurfaceHandle);
    ...
    public virtual void SetOverlay()
    {
        ...
        // SetWindowLong(
        //     OverlayHandle,
        //     GetWindowLongEx(OverlayHandle) | WindowStylesEx.WS_EX_TRANSPARENT
        // );
        
        // var exStyle = GetWindowLongEx(OverlayHandle);
        //
        // SetWindowLong(
        //     OverlayHandle,
    ...
    {   // NOTE: Here we work on DPIs -not physical- pixels | TODO: Fix prev -> new ratio change for wished size
        ...
        if (_IsAttached)
        {
            ...
            screen          = new(Utils.PointEmpty, Owner.RenderSize);
